...
#define SECONDARY_SCK 14  // Changed to match primary SCK
#define SECONDARY_WS 15   // Changed to match primary WS
#define SECONDARY_SD 33

// Volume control
#define VOLUME_GAIN 3     // Amplification factor (1-8)
#define MIC1_GAIN 1.2     // Individual mic sensitivity
#define MIC2_GAIN 1.2     // Individual mic sensitivity
...
void setup() {
  ...
  i2s_config_t primary_config = {
    ...
    .dma_buf_len = 512,
    .use_apll = true       // High precision clock
  };
  ...
  i2s_config_t secondary_config = {
    ...
    .sample_rate = 44100,   // Must match primary
    .bits_per_sample = I2S_BITS_PER_SAMPLE_16BIT,
    .channel_format = I2S_CHANNEL_FMT_ONLY_LEFT,
    .communication_format = I2S_COMM_FORMAT_STAND_I2S,
    .intr_alloc_flags = ESP_INTR_FLAG_LEVEL1,
  ...
  // Secondary pin config - MUST share clock lines!
  i2s_pin_config_t secondary_pins = {
    .bck_io_num = SECONDARY_SCK,  // Same as primary
    .ws_io_num = SECONDARY_WS,    // Same as primary
...
void loop() {
  ...
  for(int i = 0; i < sample_count; i++) {
    // Apply individual mic gains
    float mic1 = mic1_samples[i] * MIC1_GAIN;
    float mic2 = mic2_samples[i] * MIC2_GAIN;
    
    // Mix with volume boost
    int32_t mixed = (mic1 + mic2) * VOLUME_GAIN / 2;
    
    // Prevent clipping
    mic1_samples[i] = constrain(mixed, -32768, 32767);
  }

  // Output amplified audio
  ...
  if(millis() - last_debug > 1000) {
    Serial.printf("Mic1: %6d | Mic2: %6d | Gain: %d\n", 
                 mic1_samples[0], mic2_samples[0], VOLUME_GAIN);
    last_debug = millis();
  }
}
